...
function FileEditor({
  ...
  const cats2 = useApi(() => fetchCategories(), []);
  ...
  const activeCategories = (cats2.data?.categories ?? []).filter((c) => c.active);
...
globalThis.fetch = async (input, init) => {
  ...
  if (path === "/api/admin/overview")
    return respond({
      totalUsers: 2,
      activeUsers: 1,
      totalPublishedFiles: 1,
      totalFiles: 2,
      totalDownloadAuthorizations: 4,
      totalCategories: 2,
      activeTokenBalance: 12,
      tokensAdded: 50,
      tokensConsumed: 38
    });
  ...
  if (userMatch) {
    ...
    return respond({
      ...
      tokenBalance: 12,
      tokenBatches: 1,
      nextTokenExpiryAt: Date.now() + 864e5,
  ...
  if (method === "POST" && path === "/api/admin/topup")
    return respond({ ok: true, id: "b1", amount: 25, expiresAt: Date.now() + 14 * 864e5 });
